...
function draw(){
    ...
    if(backgroundImg){
        ...
        textSize(20)
        fill("yellow")
        text("Score : " + score, width-100, 150); 
        
        if(birds.length>0){
            text("Press Space Key for Next Bird", width/2+300, 25); 
            text("Bird :  "+birds.length,width/2+450, 60)
           
        }
        else{
            text("Click on 'Reload Button' to reload the Game Level",width/2-100, 50)
        }
        ...
        if(birds.length>0){
            ...
            text("Click on 'Reload Button' to reload the Game Level",width/2-200, 50)
        }
...
async function getBackgroundImg(){
    ...
    if(hour>=06 && hour<=19){
        ...
        Halloween = "sprites/Halloween.jpg";
    }
